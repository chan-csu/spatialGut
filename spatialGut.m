...
%
% INPUTS: 
%    model:          COBRA community model with the additional fields `indCom` and `infoCom`
%    species:        abbreviations or Ids of species to shut down
%
% OUTPUT:
%    modelK:         updated model with the designated species inactive
%
% EXAMPLE:
%    Create a model with only the first species active, all other being inactivated.
%    modelK = killSpecies(model, setdiff(1:numel(model.infoCom.spAbbr), 1));
